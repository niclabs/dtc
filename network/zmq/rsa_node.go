...
func (node *Node) sendRSAKeyShare(id string, key *tcrsa.KeyShare, meta *tcrsa.KeyMeta) (*message.Message, error) {
	...
	msg, err := message.NewMessage(message.SendRSAKeyShare, node.getID(), []byte(id), keyBinary, metaBinary)
	if err != nil {
		return nil, err
	}
...
func (node *Node) getRSASigShare(id string, doc []byte) (msg *message.Message, err error) {
	msg, err = message.NewMessage(message.GetRSASigShare, node.getID(), []byte(id), doc)
	if err != nil {
		return nil, err
	}
...
func (node *Node) deleteRSAKeyShare(id string) (*message.Message, error) {
	msg, err := message.NewMessage(message.DeleteRSAKeyShare, node.getID(), []byte(id))
	if err != nil {
		return nil, err
	}
	_, err = node.socket.SendMessage(msg.GetBytesLists()...)
